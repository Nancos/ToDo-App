...
// MARK: - Load Task and exit to Main screen -
private extension TaskEditViewController {
    func loadTaskData() async {
        if let id {
            await fetchTaskData(for: id)
        } else {
            id = await presenter.createNewTask()
            await fetchTaskData(for: id)
        }
    }
    
    func fetchTaskData(for id: Int?) async {
        Task {
            await presenter.fetchTask(id: id)
        }
    }
    ...
    @objc func toMainScreen() {
        doneButtonTapped()
        Task {
            await presenter.checkEmptyTask(id)
            onTaskDeleted?()
...
extension TaskEditViewController: UITextViewDelegate, UITextFieldDelegate {
    ...
    func textViewDidEndEditing(_ textView: UITextView) {
        Task {
            await presenter.updateTask(id: id, newDescription: textView.text)
        }
        closeDoneButton()
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        Task {
            await presenter.updateTask(id: id, newTitle: textField.text)
        }
        closeDoneButton()
    }
