...
final class TasksViewController: UIViewController {
    ...
    init(_ isFirstLaunch: Bool, sendData: ((String) -> Void)? = nil) {
        ...
        setupUI()
        Task { await presenter.fetchTasks(isFirstLaunch) }
    }
...
extension TasksViewController: UISearchBarDelegate {
    ...
    @objc private func executeSearch() {
        ...
        if searchText.isEmpty {
            Task { await presenter.regularLaunch() }
        } else {
            Task { await presenter.searchTasks(with: searchText) }
        }
    }
...
extension TasksViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.onCheckMarkTapped = { [weak self] id in
            ...
            Task {
                await self.presenter.updateCompletedStatus(id: id, completed: self.tasks[indexPath.row].completed)
            }
    ...
    func updateTableData() {
        Task {
            if let searchText = searchBar.text, !searchText.isEmpty {
                await presenter.searchTasks(with: searchText)
            } else {
                await presenter.regularLaunch()
            }
        }
    }
}
...
extension TasksViewController {
    ...
    private func shareTaskAction(_ indexPath: IndexPath) {
        ...
        Task {
            let sharedString = await presenter.getSharedString(with: cell.id)
    ...
    private func removeTaskAction(_ indexPath: IndexPath) {
        ...
        if let cell = tableView.cellForRow(at: indexPath) as? TaskViewCell {
            if let index = tasks.firstIndex(where: { $0.id == cell.id }) {
                ...
                Task {
                    await self.presenter.removeTask(id: taskId)
                }
            }
        }
        tableView.reloadData()
